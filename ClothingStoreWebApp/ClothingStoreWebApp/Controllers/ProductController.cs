    ...
    public class ProductController : Controller
    {
        ...
        private void ProcessProductImages(int productId, HttpPostedFileBase[] images)
        {
            try
            {
                ...
                var existingImages = db.ProductImages.Where(pi => pi.ProductID == productId).ToList();
                int sortOrder = existingImages.Any() ? existingImages.Max(pi => pi.SortOrder) + 1 : 1;
                ...
                foreach (var image in images.Where(img => img != null && img.ContentLength > 0))
                {
                    ...
                    {
                        System.Diagnostics.Debug.WriteLine($"Skipped file: {image.FileName} - Invalid extension");
                        continue;
                    }
                    ...
                    try
                    {
                        ...
                        string fileName = $"product_{productId}_{DateTime.Now.Ticks}_{sortOrder}{extension}";
                        ...
                        {
                            System.Diagnostics.Debug.WriteLine($"Failed to save file: {filePath}");
                            continue;
                        }

                        // ✅ LỖI ĐÃ SỬA - Đường dẫn chính xác + Property chính xác
                        var productImage = new ProductImage
                        {
                            ProductID = productId,
                            ImageURL = "/Content/Images/Products/" + fileName,  // ✅ Viết hoa chính xác
                            SortOrder = sortOrder,
                            ImageType = "Product",
                    ...
                    {
                        System.Diagnostics.Debug.WriteLine($"Error saving individual image: {fileEx.Message}");
                        continue;
                    }
                }

                // Save all images to database at once
                if (db.ProductImages.Local.Count > 0)
                {
                    db.SaveChanges();
                }
            }
            catch (Exception ex)
            {
                ...
                throw;  // Throw exception để biết lỗi xảy ra ở đâu
            }
        }
        ...
        [HttpPost]
        [Authorize]
        public JsonResult SetMainImage(int imageId, int productId)
        {
            try
            {
                if (!IsAdmin())
                ...
                var allImages = db.ProductImages.Where(pi => pi.ProductID == productId).ToList();
                foreach (var img in allImages)
                    img.SortOrder = 1;

                var mainImage = db.ProductImages.FirstOrDefault(pi => pi.ImageID == imageId && pi.ProductID == productId);
                if (mainImage != null)
                {
                    mainImage.SortOrder = 0;
                    db.SaveChanges();
                    return Json(new { success = true, message = "Ảnh chính đã được cập nhật!" });
                }
        ...
        [HttpPost]
        [Authorize]
        public JsonResult DeleteProductImage(int imageId, int productId)
        {
            try
            {
                if (!IsAdmin())
                ...
                var image = db.ProductImages.FirstOrDefault(pi => pi.ImageID == imageId && pi.ProductID == productId);
                if (image != null)
                {
                    try
                    {
